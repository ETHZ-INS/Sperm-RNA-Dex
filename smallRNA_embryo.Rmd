...
```{r, message=FALSE}
study_info <- data.table(read_delim("~/Google_Drive/Results/Kathi/study_info.txt", "\t", escape_double = FALSE, trim_ws = TRUE))

sample_table <- data.table(read_csv("~/Google_Drive/Results/Kathi/sample_table.csv"))
...
TOTAL_gene_count <- data.table(read_delim("~/Google_Drive/Results/Kathi/TOTAL.gene_count.txt", delim="\t", comment="# "))
...
#total_genes <-  data.table(getBM(attributes=c('ensembl_gene_id','ensembl_transcript_id', 'entrezgene', "wikigene_description", "mgi_symbol"),filters = 'ensembl_gene_id', values = vapply(strsplit(TOTAL_gene_count$Geneid, ".", fixed = TRUE), "[", "", 1) , mart = ensembl))
...
#total_genes <- data.table(total_genes)
...
```{r}
#ERCC_mit_Gene_counts[, cell:=substring(cell, 12, 22)]
...
#TOTAL_gene_count_melted_filtered <- TOTAL_gene_count_melted[cell %in% ERCC_mit_Gene_counts[total_count>=5e5  & ERCC_percentage < 10 & type=="Gene", cell], ]


TOTAL_gene_count_melted_filtered <- TOTAL_gene_count_melted[!cell %in%  filtered_out, ]

#TOTAL_gene_count_melted_filtered[, cell:=sapply(strsplit(as.character(cell),"/"), `[`, 2)]
#TOTAL_gene_count_melted_filtered[, cell:= gsub('.bam', '',  cell ) ]
...
#gene_count_monocle <- TOTAL_gene_count_filtered_matrix[which(rowSds(TOTAL_gene_count_filtered_matrix)!=0),] 
...
```{r}
gene_table <- data.table(getBM(attributes=c('ensembl_gene_id_version','ensembl_gene_id', 'entrezgene', "wikigene_description", "mgi_symbol"),filters = 'ensembl_gene_id_version', values = rownames(TOTAL_gene_count_filtered_matrix) , mart = ensembl))
...
write.table(gene_markers[sort(-sc3_2_markers_auroc)], file = "./cluster_marker_info.txt", append = FALSE, quote = F, sep = " ",
...
write.table( gene_DE , file = "./cluster_DE_info.txt", append = FALSE, quote = F, sep = "\t",
            eol = "\n", na = "NA", dec = ".", row.names = F,
            col.names = TRUE, qmethod = c("escape", "double"),
            fileEncoding = "")
...
#gene_table$gene_short_name <- gene_table$gene_name

pd <- new("AnnotatedDataFrame", data = metadata_filtered)
fd <- new("AnnotatedDataFrame", data = gene_table)
HSMM <- newCellDataSet(gene_count_monocle, phenoData = pd, featureData = fd, expressionFamily=tobit() )
...
#HSMM1 <- newCellDataSet(TOTAL_gene_count_filtered_matrix, phenoData = pd, featureData = fd, expressionFamily=tobit() )
#HSMM2 <- newCellDataSet(TOTAL_gene_count_filtered_matrix, phenoData = pd, featureData = fd, expressionFamily=tobit() )
...
cat(expressed_genes_mart$mgi_symbol, sep="\n")
...
input.ERCC.annotation<-read.delim("~/Google_Drive/Results/Kathi/ERCCs/ERCC_Controls_Analysis.txt", header=T)
...
ERCC_Controls_Annotation <- read_delim("~/Google_Drive/Results/Kathi/ERCCs/ERCC_Controls_Annotation.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE)

ERCC_Controls_Analysis <- read_delim("~/Google_Drive/Results/Kathi/ERCCs/ERCC_Controls_Analysis.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE)
